...
export function evaluateDeterministicEgress({ toolName, params, sessionTaint = "clean" }) {
  ...
  if (
    ...
    return {
      ...
      blockReason:
        "OpenClaw Scanner blocked this tool call because it targets a secret or host-sensitive path.",
    };
  }
  ...
  if (capability === "shell_exec" && paths.some((candidate) => SECRET_PATH_PATTERNS.some((pattern) => pattern.test(candidate)))) {
    return {
      ...
      blockReason:
        "OpenClaw Scanner blocked this command because it appears to access secrets or host-sensitive files.",
    };
  }
